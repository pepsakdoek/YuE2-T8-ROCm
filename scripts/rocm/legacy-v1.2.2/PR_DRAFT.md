...
> 隐含 24GB 卡假设。实测同一 175s 歌曲在 16GB 卡上：
> - `vae_core_frames=1024`：VAE 解码 **313s**（总 613.7s）
> - `vae_core_frames=512` ：VAE 解码 **~156s**（预期总 ~457s，**约快 25%**）
> （分块基准，同一段 60s latent：512→101.6s / 1024→230.9s / full→285.0s）
...
对 24GB NVIDIA 用户是零变化；对 16GB 用户是 25% 提速；API/前端可选择性透传。

**待办（提交前必须做）**：在我们这台机器上复测一次，拿到 457s 的实测数
（补丁已应用，worker 每次作业新起进程即生效）。
