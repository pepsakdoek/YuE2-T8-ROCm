...
def load_models(args):
    ...
    local_model_root = os.environ.get("SEED_VC_MODEL_ROOT")
    if local_model_root:
        local_model_root = os.path.abspath(local_model_root)
        config["model_params"]["vocoder"]["name"] = os.path.join(
            local_model_root, "bigvgan_v2_44khz_128band_512x"
        )
        config["model_params"]["speech_tokenizer"]["name"] = os.path.join(
            local_model_root, "whisper-small"
        )
